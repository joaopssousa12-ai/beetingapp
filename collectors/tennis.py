...
def insert_matches(rows):
    ...
    if USE_POSTGRES:
        # Batch insert via execute_values — ~100x faster than row-by-row over network
        import psycopg2.extras
        sql = f"INSERT INTO tennis_matches ({col_names}) VALUES %s ON CONFLICT DO NOTHING"
        ...
        try:
            ...
            print(f"Tennis batch insert error: {e}", flush=True)
            try:
                conn._conn.rollback()
            except Exception:
                pass
